...
export async function beschriften(bild, box, name, text) {
  ...
  // Die Flaeche sitzt eng um den Satz statt auf fester Hoehe: bei einem
  // kurzen Spruch bliebe sonst unten Luft.
  const glasHoehe = satz.hoch + rand * 2;
  const glasOben = h * box.y - rand + Math.max(0, h * box.h - satz.hoch) / 2;

  const glas = await milchglas(sharp, bild, b, h,
    b * box.x - rand, glasOben, b * box.w + rand * 2, glasHoehe);
  const svg = satzSVG(b, h, box, satz, glasOben + rand);
