...
// Was in jedem Prompt steht, egal welche Kategorie.
//
// Das Hochformat steht bewusst NICHT mehr im Prompttext: Jeder Anbieter bekommt
// das Seitenverhaeltnis als eigenen Parameter. Stand es zusaetzlich im Text,
// haben Modelle daraus eine Bildkomposition gemacht - im Vergleichslauf kam
// dabei einmal derselbe Sprecher zweimal uebereinander heraus.
//
// Schrift bleibt verboten: Buchstaben malt jedes Videomodell fehlerhaft, der
// Text kommt spaeter in WhatsApp dazu.
export const NEGATIV =
  "text, letters, numbers, subtitles, captions, watermark, logo, split screen, " +
  "collage, multiple panels, duplicated person, distorted faces, extra fingers, " +
  "blurry, low quality";

const GRUNDREGELN =
  "One single continuous shot, one subject, no split screen and no repeated " +
  "figures. Absolutely no text, letters, numbers, subtitles, signs, watermarks " +
  "or logos anywhere in the frame.";
...
// --- Szenenplan fuer laengere Videos --------------------------------
//
// Fuenfzehn Sekunden derselben Einstellung sind kein besseres Video, sondern
// ein langweiligeres. Was Kunden meinen, wenn sie "laenger" sagen, ist fast
// immer "mehr zu sehen". Deshalb entstehen aus denselben vier Antworten drei
// Einstellungen: hinfuehren, Botschaft, Abschluss - der Aufbau, den jeder
// Werbespot hat.
//
// Kling verlangt, dass die Summe der Dauern genau der Gesamtlaenge entspricht.
export function szenenPlan(briefing = {}, kategorieId = "sprecher", sekunden = 15) {
  const ton = TON[briefing.tonId] || TON.herzlich;
  const ort = BRANCHE[briefing.brancheId] || BRANCHE.sonstiges;
  const satz = (briefing.botschaft || "").trim().replace(/"/g, "'");

  // Drei moeglichst gleich lange Abschnitte, Rest auf die mittlere Szene.
  const a = Math.floor(sekunden / 3);
  const dauern = [a, sekunden - 2 * a, a];

  const bilder = kategorieId === "produkt"
    ? [
        `Establishing shot of ${ort}, calm and inviting, no people. Lighting: ${ton.licht}.`,
        `The product in the centre of the frame, slow circling camera. What it is about: ${satz}`,
        `A close detail of the product, then the camera settles. Mood: ${ton.stimmung}.`,
      ]
    : kategorieId === "anlass"
      ? [
          `Establishing shot of ${ort}, decorated for the occasion, small lights, gentle movement.`,
          `People arriving and greeting each other, joyful. The occasion: ${satz}`,
          `The camera rises slowly over the celebration. Mood: ${ton.stimmung}.`,
        ]
      : kategorieId === "charakter"
        ? [
            `A strongly caricatured human character in the look of a modern 3D animated film enters ${ort}.`,
            `The character acts out what this is about: ${satz}`,
            `The character looks triumphantly into the camera. Mood: ${ton.stimmung}.`,
          ]
        : [
            `Establishing shot of ${ort}, warm and inviting. Lighting: ${ton.licht}.`,
            `A friendly, natural-looking person speaks directly into the camera in German and says: "${satz}"`,
            `The same person smiles and nods once, then the shot holds. Mood: ${ton.stimmung}.`,
          ];

  return bilder.map((prompt, i) => ({
    prompt: `${prompt} ${GRUNDREGELN}`,
    duration: dauern[i],
  }));
}
