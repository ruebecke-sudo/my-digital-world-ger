...
export async function textAufbringen(bildBuffer, motif, name, text) {
  ...
  if (eigenes) {
    ...
    // sie geschrieben hat.
    const bezeichnung = (motif.bezeichnung || "")
      .trim()
  ...
  const motivRein = eigenes && motif.bezeichnung
    ? { ...motif, bezeichnung: (motif.bezeichnung || "").trim().replace(/\s*[-–—]?\s*des\s+jahres\.?$/i, "").trim(), kurz: (motif.bezeichnung || "").trim().replace(/\s*[-–—]?\s*des\s+jahres\.?$/i, "").trim().toUpperCase() }
    : motif;
