...
export function buildPrompt(motif, name, text) {
  ...
  const schema = motif.textSchema || CATALOG.textSchema;
  const stil = motif.stylePrompt || CATALOG.stylePrompt;

  const textTeil = schema
  ...
  return `${stil} Szene: ${motif.szene} ${textTeil} Alle Texte in fehlerfreiem Deutsch, exakt wie angegeben.`;
}
