...
// 2. Motiv ohne festes Bild: Die KI liefert das Bild ohne Schrift, die
//    komplette Typografie entsteht hier als Vektorebene.
//
// In beiden Faellen steht der Text garantiert korrekt im Poster - er wird
// nicht von der Bild-KI gemalt.
export async function textAufbringen(bildBuffer, motif, name, text) {
  const spruch = (text && text.trim()) || motif.defaultText || "";

  if (motif.textBereich) {
    return await beschriften(bildBuffer, motif.textBereich, (name || "").trim(), spruch);
  }
