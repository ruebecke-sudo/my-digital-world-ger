...
function bloecke(d, B, rand, spalte) {
  ...
  saetze.forEach((satz, si) => {
    ...
    // Findet sich im Satz kein gepflegtes Schluesselwort, wird im letzten Satz
    // das letzte laengere Wort hervorgehoben. Sonst blieb der ganze Lauftext
    // weiss - die meisten Motive haben gar keine Akzente hinterlegt, und der
    // Kunde schreibt ohnehin seinen eigenen Text.
    if (!woerter.size && letzter) {
      const kandidat = satz.split(/\s+/).map(w => w.replace(/[^\p{L}\p{N}-]/gu, ""))
        .filter(w => w.length >= 5).pop();
      if (kandidat) woerter.add(kandidat.toLowerCase());
    }
...
export function textebeneSVG(breite, hoehe, motif, name, text, spaltenAnteil = 0.46, neigung = -2.6) {
  ...
  const defs = `<defs><linearGradient id="gold" x1="0" y1="0" x2="0" y2="1">
    ...
    <stop offset="1" stop-color="#d8890a"/></linearGradient>` +
    // Dunkler Verlauf hinter der Schrift. Kontur und Schatten allein reichen
    // nicht, wenn die KI links einen hellen Hintergrund malt.
    `<linearGradient id="schirm" x1="0" y1="0" x2="1" y2="0">` +
    `<stop offset="0" stop-color="#000" stop-opacity="0.72"/>` +
    `<stop offset="0.5" stop-color="#000" stop-opacity="0.42"/>` +
    `<stop offset="0.85" stop-color="#000" stop-opacity="0.08"/>` +
    `<stop offset="1" stop-color="#000" stop-opacity="0"/></linearGradient></defs>` +
    `<rect x="0" y="0" width="${breite}" height="${hoehe}" fill="url(#schirm)"/>`;
